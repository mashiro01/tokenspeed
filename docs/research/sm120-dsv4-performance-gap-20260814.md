...
6. **本轮 TP4/DCP2 已跨过 correctness gate，A/B/C 优化实测有效，但剩余差距仍由通信/launch latency 主导。** exact-1M 容量 receipt 为 6.85 GB/rank；2-GPU NCCL CUDA graph primitive 64/64 replay 通过；4095-token prompt 固定输出 64 tokens 的 20 次服务请求 hash 全同且与 DCP1 相同。静态 sink、packed candidate exchange 和 fused LSE/RS-layout writer 已把约 214 次 DCP collective/token 降到约 150 次，并把 merge 的约 10 个 PyTorch CUDA op 收敛为一个 kernel。4095/256 的 C1 server decode 中位数由 47.0 提升到 51.7 tok/s（+10.0%），端到端中位数由 44.2 提升到 48.42 tok/s（+9.55%）。该结果验证了延迟诊断，但尚不足以追回新栈 DCP1 的约 80 tok/s；下一结构性阶段是 DCP-group IPC 和 peer-read state merge。
...
| A/B/C 前端到端 C1 decode | 同一 TP4/DCP2 服务、同一实际 4095-token prompt、固定输出 256 tokens、单并发；沿用 matched harness 的 decode-only 口径，不含 TTFT；连续 3 次完整请求 | **46.9 / 47.0 / 47.8 tok/s**，平均 47.23、median 47.0 tok/s | 这是 A/B/C 前的 DCP2 性能基线，不是目标值。此前同栈 DCP1 热态约 79.97 tok/s、53 的 vLLM TP4/DCP2 约 107 tok/s 只能作为方向性参照；由于 feature stack/backend 不完全相同，不能把差值全部归因于 DCP degree。 |
| A/B/C 后端到端 C1 decode | 与上一行相同的 TP4/DCP2、4095/256、单并发合同；镜像 `tokenspeed:sm120f-dcp-abc-7bd962e`，连续 3 次完整请求 | server decode **50.9 / 51.7 / 51.8 tok/s**，平均 51.47、median 51.7；端到端 **47.69 / 48.42 / 48.43 tok/s**，median 48.42 | 相对基线 server mean/median 为 **+8.97% / +10.0%**，端到端 median 为 **+9.55%**。说明减少小 collective 和 launch 有真实收益；仍不能把剩余差距解释为模型上限或 KV 容量。 |

本轮还定位了一个独立的 **multi-row correctness root cause**。旧 `merge_dcp_attention_states()` 先取 `reduce_scatter_input[:, :rows]`，再执行 `permute(1, 0, 2, 3).reshape(rows, group_heads, head_dim)`，把结果当作 `torch.mul(..., out=corrected)` 的 destination。当 `rows > 1` 时，该 permute 后的 stride 无法按目标 shape 表示为 view，`reshape` 会物化临时 tensor；乘法写进临时 tensor，而后续 NCCL reduce-scatter 读取的原 `reduce_scatter_input` 没有被写入。`rows=1` 恰好可形成 view，所以普通 C1 decode 会掩盖问题。`c99217c` 先按 destination rank 显式 view/permute 并直接写原 RS staging；A/B/C 随后用 `dcp_attn_merge_rs` 完全替换这段 PyTorch writer。上述 2-GPU、multi-row、64-replay test 是共同的 graph correctness gate。这个 bug 解释旧 multi-row 错误，**不解释当前 C1 仍只有约 52 tok/s**。
...
|---|---|---|---|
| A：静态 sink（已完成） | 利用 checkpoint loader 已经看到完整 `attn_sink` 的事实，为每层保留完整 global sink；owner rank 直接取所属 DCP subgroup view，非 owner 使用预分配全 `-inf` tensor。没有 graph capture 中的懒初始化或每步 copy/fill。 | 去掉 43 次 sink all-gather，约 `214 → 171` collectives/token。 | DCP1/DCP2 sink head slice、owner 语义、全层输出 hash；capture 前地址固定。 |
| B：合并 indexer candidate（已完成） | 把一个 candidate 的 score bits 与 global row 组成一个 8-byte record，一次 all-gather 后按 rank-major 物化；不改变 global top-k 语义。 | 每个 c4 层 2 次 all-gather 变 1 次，再少 21 次，约 `171 → 150` collectives/token。 | FP32 score bit-exact、无效 row、rank-major row 对应、prefill/decode 和 graph replay。 |
| C：融合 LSE 与 RS-layout writer（已完成） | `tokenspeed-kernel` 新增严格无 fallback 的 `dcp_attn_merge_rs`：从 gathered LSE 做 nan-safe merge，计算本 rank partial weight，并直接写 destination-major RS input，同时写 merged LSE。runtime 仍保留 NCCL LSE all-gather 与 RS。 | collective 数仍约 150，但每层约 10 个小 CUDA op 收敛为 1 个，并从结构上固定 multi-row RS layout。 | DCP2/4、BF16/FP16、rows 1/多行、head dim 512、NaN/+inf/-inf/all-`-inf`、固定 capacity workspace 与 64 次 graph replay。 |
...
A+B 合计移除 64/214、约 30% 的 DCP collective；C 再把每层 merge writer 的约 10 个 launch 收敛为一个。三项由 `31774489` 集成，`7bd962eb` 生成运行镜像。SM120 上 kernel gate 为 5/5，通过 DCP2/DCP4、BF16/FP16、head dim 512、固定 capacity CUDA graph replay 和 all-empty/NaN；runtime math、packed candidates 与真实双卡 NCCL graph gate 为 20/20。

实际服务资格使用 exact `max_model_len=1,048,576`、TP4/DCP2、GPU0--3、FP8 KV 与 MXFP4 experts。4095/64 连续 20 次输出 hash 均为 `bbc37393d8dc325187adf857950b5c32914f72824aed5e4d5e36a376626c9e2f`，与优化前 DCP2 和 matched DCP1 完全一致；热态端到端中位延迟由约 1.67 s 降至 1.579 s（-5.4%）。4095/256 连续三次输出 hash 也全部一致；server decode mean/median 由 47.23/47.0 提升为 51.47/51.7 tok/s（+8.97%/+10.0%），端到端 median 由 44.2 提升为 48.42 tok/s（+9.55%）。TTFT 为 346--352 ms，prefill 为 320--323 ms，因此该增益发生在目标 decode hot path，而不是靠改变 1M 服务合同或预填充口径。

这组数据与事前约 53 tok/s 的延迟量级估计一致，也把下一步收得更窄：A/B/C 值得保留，但仅靠 NCCL 上层减少 launch 无法追回 DCP1 的约 80 tok/s。D/E 必须直接处理 DCP subgroup 的小报文 IPC 固定成本，并把 LSE/partial output 的跨 rank 交换与 merge 合并；继续调整 KV 容量、换机器或只调 MoE tactic 都不会解释剩余差距。后续仍要分别记录 Q gather、attention kernel、LSE merge、RS 和 indexer candidate gather 的 CUDA-event/NVTX 时间，不能只看一个端到端 tok/s 猜根因。
